...
app.get('/get-comments/:pageName', function (req, res) {
   ...
   pool.query('SELECT comment.*, "user".username FROM page, comment, "user" WHERE page.title = $1 AND ID = comment.article_id AND comment.user_id = "user".id', [req.params.pageName], function (err, result) {
      if (err) {
          res.status(500).send(err.toString());
      } else {
